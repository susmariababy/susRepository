	         ...
	         public void actionPerformed(ActionEvent e) { //onClick of the search button
	        	 if (tst.searchTST(tst.root, textField.getText().toLowerCase()) != null) {
					...
					{ //here we have to display the spellCheck suggestions for the word
						...
						NodeFunctions.spellCheck(BKroot,textField.getText().toLowerCase() , 3,misSpelt,neighbour);
						ArrayList<String> list = new ArrayList<String>();
						for(int i=0;i<misSpelt.size();i++){
							list.add(misSpelt.poll().word); // add the words of the priority Queue to a list
						}
						
						data = new Vector<String>(list);
						 
						  mPanel.setVisible(false);//I will be using the moPanel to display the spellCheck suggestions
						  moPanel.removeAll();
						  moPanel.setVisible(false);
					      sPanel.setVisible(false);
					      soPanel.setVisible(false);
